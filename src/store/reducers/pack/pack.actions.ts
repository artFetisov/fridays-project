...
export const getAllPacksTC = createAsyncThunk<
  void,
  { urlHasQueryParams?: boolean },
  { state: AppRootState }
>('pack/getAll', async ({ urlHasQueryParams }, { dispatch, getState }) => {
  try {
    ...
    if (
      (maxCardsCount !== response.maxCardsCount || minCardsCount !== response.minCardsCount) &&
      urlHasQueryParams === undefined
    ) {
...
export const createPackTC = createAsyncThunk<void, ICreatePackData, { state: AppRootState }>(
  'pack/create',
...
export const updatePackTC = createAsyncThunk<void, IUpdatePackData, { state: AppRootState }>(
  'pack/update',
  async (cardsPack, { dispatch }) => {
    try {
      dispatch(setAppStatus('loading'))
...
export const deletePackTC = createAsyncThunk<void, { packId: string }, { state: AppRootState }>(
  'pack/delete',
